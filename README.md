...
- **산출물 주제:**
- **제작 목적:** 물리적 제약이 있는 기존 발표 환경을 해결하기 위해, 이동의 자유를 보장하는 동시에 청중과의 실시간 상호작용을 돕는 크로스 플랫폼 발표/강의 보조 솔루션을 구축하는 것이 목적이다. 스마트폰 하나로 슬라이드 제어, 발표 대본 및 시간 확인을 통합 관리하며, 실시간 질문 수집을 통해 발표자와 청중 간의 양방향 소통을 극대화한다.
- **선택 옵션:** 실시간 인터랙션 & Cross-Platform
- **핵심 구현 요소:**
  **[발표자]**
  - **프레젠테이션 제어**
    - 다음/이전 슬라이드 전환
    - 다인원 발표 시 슬라이드 컨트롤 권한 자동 전환
  ...
  **[청중]**
  ...
  - **실시간 질문**
    - 공개 여부를 선택하여 실시간으로 질문 등록
  **[PC]**
  - **슬라이드 동기화**
    - 모바일에서 보낸 신호를 받아 실제 화면 전환
  - **질문 목록 표시**
    - 모인 공개 질문 목록을 화면에 표시 가능
  - **발표 자료 자동 로드**
    - 해당 발표자가 사전 업로드한 파일을 클라우드에서 자동으로 불러와 화면을 즉시 전환
- **사용 / 시연 시나리오:**
...
- **팀원별 역할:**
  - 김민:  PC 웹 뷰어, 데이터베이스 설계 및 연동, 권한/상태 관리, AI 연동 담당
  - 김규민: 모바일 앱 UI/UX, 클라이언트 로직, 파일 업로드 UI 담당
